...
- [ ] **Session 16 — Critical fixes to testing companion before wider testing:**
  - [ ] Notes vs Thoughts label confusion — clearer distinction, inline explanations
  - [ ] Pause-and-return — explicit "Resume now" affordance; clearer return instructions on pause screen
  - [ ] Frequency scale visual — smaller blocks, brighter labels, legible selected-value text
  - [ ] Q42 placeholder honesty — replace with gist text until full build
  - [ ] Grey text legibility throughout — accessibility fix on `--text-dim` and `--text-dimmer`
  - [ ] Notes persistence + "View my saved notes" screen — Notes always persist regardless of save-mode
  - [ ] Q39 "unexplained" wording — rewrite
  - [ ] Audit and remove "ND" / audience-naming throughout questions, section names, intros, and tagline framing
